...
void main()
{
	...
	gbuffer_colour = vec4(texture(colour_map,EdgeCoordinates).rgb,0);
	gbuffer_position = vec4(Position,0);

	// translate normals
	vec3 normals = texture(normal_map,EdgeCoordinates).rgb*2.0-1.0;
	gbuffer_normals = vec4(normalize(TBN*normals),.0);

	// extract surface materials
	gbuffer_materials = vec4(texture(material_map,EdgeCoordinates).rgb,.0);
}
